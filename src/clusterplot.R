...
## Plot clustering
ui = sort(unique(i)); K = length(ui); Ncol <- K;
clustercols <- rainbow(K);
plot(xRange, yRange, type = 'n', xlab = xlab, ylab = ylab, ...)

for (k in 1:K) {
    points(X[i == ui[k],1], X[i == ui[k],2], col = clustercols[((k - 1) %% Ncol) + 1],
           lwd = 6);
}
...
if (length(centroids) != 0) {
    ...
    for (k in 1:dim(centroids)[1]) {
        points(centroids[k, 1], centroids[k, 2], pch = 4,
               col = clustercols[((k - 1) %% Ncol) + 1], cex = 3, lwd = 2);
    }
}

uy = sort(unique(y)); C = length(uy); classcols <- rainbow(C+2)[3:(C+2)];
Ncol <- C;
## Plot class labels
for (k in 1:C) {
    points(X[y == uy[k], 1], X[y == uy[k], 2],
           col = classcols[((k - 1) %% Ncol) + 1], pch = 20);
}
}  ## End of clusterplot
