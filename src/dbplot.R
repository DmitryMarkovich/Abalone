dbplot <- function(X, attributeNames, predictionFunction, y,
                   contourLevels = 0.5, contourCols = 'white', cols = c(), ...) {
    ...
    ##    cols: colors used to shade the areas belonging to the
    ##    different classes according to the prediction function. Only
    ##    used in the non-binary case. In the binary case, the colors
    ##    used are always blue and green.
    ##    If the number of attributes is M=2, the decision boundary is
    ##    plotted in the attribute space. Otherwise it is plotted in
    ...
    if (all(0 <= probs && probs <= 1)) {
        image(seqx, seqy, t(probs), col=rev(rainbow(200, start=2/6, end=4/6)),
              xlab=attributeNames[1], ylab=attributeNames[2], main="Decision boundary")
        cols <- c('green', 'blue')
    } else {
        image(seqx, seqy, t(probs), col=rev(cols), xlab=attributeNames[1],
              ylab=attributeNames[2], main="Decision boundary")
        contour(seqx, seqy, t(probs), levels=contourLevels, labels="", add=TRUE,
                type='l', lwd=5, col=contourCols)
    }
