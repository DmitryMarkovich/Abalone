plot.nnet <- function(model) {
    ## Plot a visual representation of the neural network contained in
    ## the input model.
    ## Author: Laura Frølich, lff@imm.dtu.dk

    tryCatch(library(neuralnet),
             finally = "The function plot.nnet depends on the package neuralnet, which was not found to be installed. Install this before using plot.nnet (or using plot to plot an object of class nnet).")
    mynames <- c("error"); myvect <- c(model$value);
    ...
    for (ireceivinghiddenname in 1:length(receivinghiddennames)) {
        mynams <- c(mynams,paste(inputnames,
                                 receivinghiddennames[ireceivinghiddenname],
                                 sep='.to.'))
    }
    ...
    class(nnobj) <- 'nn'
    plot(x = nnobj, information = TRUE, cex = 2);
}
