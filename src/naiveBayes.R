...
featureValueObservations <- function(observedFeatVals, possibleFeatVals) {
    return(sapply(possibleFeatVals,
                  FUN = function(x, observedFeatVals) {
                      sum(x == observedFeatVals)
                  },
                  observedFeatVals = observedFeatVals)
           );
}  ## End of featureValueObservations
################################################################################
...
naiveBayes <- function(X, y, distribution = 'mvmn', prior = 'empirical') {
    ...
    ## distribution: The distributions of features. Can either be a
    ## single string or a vector of strings with length equal to the
    ## number of features. Each string then gives the distribution for
    ## a feature. The allowed distributions are "mvmn" for
    ## multivariate multinomial and "normal" for the normal
    ## distribution. The default is "mvmn".
    ## prior: The prior on classes. Can either be the string "uniform"
    ## (for equal priors for all classes) or "empirical" (for class
    ## priors equal to class proportions in training data). The default
    ## is "empirical".
    ...
    ## mvmnFeatsBinary: A binary vector with length equal to the
    ## number of features. It has the value TRUE at indices where the
    ## feature follows the multivariate multinomial distribution and
    ## is otherwise false.
    ## normalFeatsBinary: A binary vector with length equal to the
    ## number of features. It has the value TRUE at indices where the
    ## feature follows the normal distribution and is otherwise false.
    ...
    if (length(distribution) == 1) {
        if(!(distribution %in% allowedDists)) {
            stop(paste('naiveBayes: the argument "distribution" must be either ',
                       paste(allowedDists, collapse=' or '), sep=''));
        }
    } else if (length(distribution) == nFeats) {
        if(!all(distribution %in% allowedDists)) {
            stop(paste('naiveBayes: each string in the argument "distribution" must be either ',
                       paste(allowedDists, collapse=' or '), sep=''));
        }
    } else {
    ...
    normalFeats <- distribution == 'normal';  ## print(normalFeats);
    mvmnFeats <- distribution == 'mvmn';  ## print(mvmnFeats);
...
########### fit paramaters
    if (any(normalFeats)) {
        for(iclass in 1:nClasses) {
            ...
            sigma <- apply(X[currentClassIndices,normalFeats], 2,
                           FUN = function(x){sd(x, na.rm=TRUE)}) # column sd's
    ...
    if (any(mvmnFeats)) {
        possibleFeatVals <- apply(X = X[, mvmnFeats, drop = FALSE], MARGIN = 2,
                                  FUN = function(x) {
                                      list(sort(unique(x)))
